...
function convertToRomanNumerals(num) {
  ...
  const units = num % 10;
  if (units in alph) return `${result}${alph[units]}`;
  return result;
}
...
function convertNumberToString(numberStr) {
  ...
  let resultWithoutSpace = '';
  for (let i = 0; i < result.length - 1; i += 1) {
    resultWithoutSpace += result[i];
  }
...
function isContainNumber(num, digit) {
  const iterableNum = `${num}`;
  for (let i = 0; i < iterableNum.length; i += 1) {
    if (+iterableNum[i] === digit) {
      return true;
    }
  }
  return false;
...
//   let matr = matrix;
//   const resMatrix = Array(matrix.length)
//     .fill(null)
//     .map(() => []);

//   for (let j = 0; j < matrix.length; j += 1) {
//     for (let i = matrix.length - 1; i >= 0; i -= 1) {
//       const resMatrixRow = j;
//       resMatrix[resMatrixRow].push(matrix[i][j]);
//     }
//   }
//   matr = resMatrix;
//   return matr;
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < mtrx.length; i += 1) {
    for (let j = i + 1; j < mtrx.length; j += 1) {
      [mtrx[i][j], mtrx[j][i]] = [mtrx[j][i], mtrx[i][j]];
    }
  }

  for (let i = 0; i < mtrx.length; i += 1) {
    for (let j = 0; j < Math.floor(mtrx.length / 2); j += 1) {
      [mtrx[i][j], mtrx[i][mtrx.length - j - 1]] = [
        mtrx[i][mtrx.length - j - 1],
        mtrx[i][j],
      ];
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  function getCycleLength(string) {
    let res = string;
    let cycleCount = 0;
  ...
  const cycleLength = getCycleLength(str);

  for (let iter = 0; iter < iterations % cycleLength; iter += 1) {
    let start = '';
    let end = '';
